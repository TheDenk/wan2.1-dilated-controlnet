...
class WanControlnetPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    r"""
    ...
    def prepare_controlnet_frames(self, controlnet_frames, height, width):
        prepared_frames = prepare_frames(controlnet_frames, (height, width))
        controlnet_encoded_frames = prepared_frames.to(dtype=self.controlnet.dtype, device=self.controlnet.device)
    ...
    def __call__(
        ...
        if controlnet_latents is None and len(controlnet_frames):
